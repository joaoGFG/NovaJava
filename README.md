...
│ ┌──────────────────┐  ┌────────────────────┐  ┌──────────────────────────┐  │
│ │ Next.js + React  │  │ React Native Mobile │  │  Static Dashboards/Apex  │  │
│ │  (deployed Azure)│  │  (Play Store/App)   │  │  (reports / admin UI)   │  │
│ └──────────────────┘  └────────────────────┘  └──────────────────────────┘  │
└──────────────────────────────────────────────────────────────────────────────┘
                                  │
                                  ▼
...
│ │  • Security: Spring Security + JWT (RSA 2048) + OAuth2 Resource Server   │ │
│ │  • Exception Handling / Global Advice                                    │ │
│ │  • Chatbot Adapter (Perplexity AI client / pluggable AI provider)        │ │
│ │  • AIContextService: user context, conversas, metas e skills             │ │
│ └──────────────────────────────────────────────────────────────────────────┘ │
└──────────────────────────────────────────────────────────────────────────────┘
                                  │
                                  ▼
...
│ ┌──────────────────────────────────────────────────────────────────────────┐ │
│ │                            Oracle Database                                │ │
│ │  • Tabelas principais: USERS, GOALS, SKILLS, AI_INTERACTIONS, CATEGORIES  │ │
│ │  • Relacionamentos: M:N (users-goals), M:N (users-skills)                 │ │
│ │  • Sequences, Constraints, Índices, Stored Procedures (quando necessário) │ │
...
│ ┌──────────────────────────────────────────────────────────────────────────┐ │
│ │  • Perplexity AI (Chatbot) — Adapter/Client com fallback provider        │ │
│ │  • Azure Services: App Service / Container Registry / SQL / Storage      │ │
│ │  • Email (SendGrid) / File Storage / Monitoring (AppInsights, Prometheus) │ │
...
│ ┌──────────────────────────────────────────────────────────────────────────┐ │
│ │  • Containerização: Docker images (backend/frontend)                      │ │
│ │  • Infra: Azure App Services / AKS / Azure SQL / KeyVault (certs/keys)    │ │
│ │  • Secrets: PERPLEXITY_API_KEY, RSA keys em KeyVault / .env (dev)         │ │
│ │  • Observability: Logs + Tracing + Health checks                          │ │
│ └──────────────────────────────────────────────────────────────────────────┘ │
